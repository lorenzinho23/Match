...
function stampa() {
  ...
  const fetch1 =
    "inserisci_dati_preda.php?username=" +
    username +
    "&sesso=" +
    sesso +
    "&gusto=" +
...
function onJSON(json) {
  if (json.length !== 0) {
    ...
    for (let i = 0; i < json.length; i++) {
      const nomePredatore = json[i].Predatore;
      const punti = json[i].Punti;
      const match_num = json[i].match_num;

      if (json[i].Punti > 0) {
        const boxNomePredatore = document.createElement("h2");
        boxNomePredatore.textContent = "Username:" + nomePredatore;
